...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _find_and_replace(self):
        ...
        target_modules_list = self.peft_config.target_modules
        if isinstance(target_modules_list, str):
            target_modules_list = [target_modules_list]

        for key in key_list:
            target_module_found = any(target_key in key for target_key in self.peft_config.target_modules)
            if not target_module_found: continue
            ...
            if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                ...
                logger.warning(
                    f"Skipping {target_name} since it is not a Linear layer and enable_lora is not set to True."
                )
                continue
    ...
    def merge_and_reinit(self):
        for module in self.model.modules():
            if isinstance(module, (Linear, MergedLinear, LoraLayer)):
