...
# Based on: https://github.com/EleutherAI/gpt-neox/blob/408e29d9c746a02d842917bb7447c5c4be0b42d4/megatron/data/gpt2_dataset.py
...
class GPT2Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if build_index_mappings:
            ...
            if self.shuffle_idx_len != self.sample_idx_len - 1:
                logger.warning(
                    f"shuffle index length ({self.shuffle_idx_len}) is not equal to sample index length ({self.sample_idx_len})"
                )
    ...
    def __getitem__(self, idx):
        try:
            return self.get_item_unsafe(idx)
        except IndexError:
            new_idx = idx % len(self)
            logger.warning(
                f"Got index out of bounds error with index {idx} - taking modulo of index instead ({new_idx})"
            )
            return self[new_idx]

    def get_item_unsafe(self, idx):
