...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    @staticmethod
    def exists(path):
        _index_exists = os.path.exists(index_file_path(path))
        if not _index_exists:
            logger.warning(f"Index file does not exist: {index_file_path(path)}")

        _data_exists = os.path.exists(data_file_path(path))
        if not _data_exists:
            logger.warning(f"Data file does not exist: {data_file_path(path)}")

        return _index_exists and _data_exists
